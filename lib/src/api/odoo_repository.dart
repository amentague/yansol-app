...
class OdooRepository<R extends OdooRecord> {
  ...
  OdooRepository(this.env, this.modelName, this.remoteRecordsCount) {
  ...
  Future<List<R>> get records async {
    latestRecords = _cachedRecords;
    env.logger.d(
        'In Odoo Repo record, $modelName: Got ${latestRecords.length.toString()} records from cache.');
    if (!_isThrottling) {
      await fetchRecords();
      _isThrottling = true;
  ...
  R createRecordFromJson(Map<String, dynamic> json) {
    return const OdooRecord(0) as R;
  }
